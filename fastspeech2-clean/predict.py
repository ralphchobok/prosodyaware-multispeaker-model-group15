...
def load_model(
    ...
    Returns:
        Tuple[torch.nn.Module, Dict, Dict]: Loaded model, words_to_phones dict, phone_to_token dict and speaker dict.
    """
    ...
    return (
        model,
        state_dict.get("words_dict", state_dict.get("pinyin_dict", {})),  # backwards compatible
...
def process_text(
    text: str, text_type: str, words_to_phones: Dict, phone_to_token: Dict
) -> Tuple[List[int], List[int]]:
    """
    Process the input text and convert it to token and stress IDs.

    Args:
        text (str): Input text.
        text_type (str): Type of input text ('text' for English text, 'phones' for phoneme sequence, 'ipa' for phonemes).
        words_to_phones (Dict): Dictionary for converting words to phones (not used for English).
        phone_to_token (Dict): Dictionary for converting phonemes to token IDs.

    Returns:
        Tuple[List[int], List[int], List[str], str]: Token IDs, stress IDs, phonemes, and phone text.
    """
    text = text.lower()
    ...
    if text_type == "text":
        # Convert English text to phonemes using G2P
        text = convert_english_to_phonemes(text)
        if not text:
            raise ValueError("Conversion to phonemes resulted in empty string")
        print(f"Phonemes: {text}")
        text_type = "phones"

    if text_type in ("phones", "ipa"):
        # Text is already in phoneme format
        token_ids, stress_ids, phonemes = convert_phones_to_tokens(phone_to_token, text)
    else:
        raise ValueError(f"Unsupported text_type: {text_type}")

    return token_ids, stress_ids, phonemes, text


def convert_english_to_phonemes(text: str) -> str:
    """
    Converts English text to IPA phonemes compatible with MFA.
    Uses g2p_en for ARPAbet conversion, then maps to IPA phonemes.

    Args:
        text (str): The input English text.

    Returns:
        str: Space-separated IPA phoneme sequence with stress markers.

    Raises:
        ImportError: If the 'g2p_en' package is not installed.
    """
    try:
        from g2p_en import G2p
    except ImportError:
        raise ImportError(
            "The 'g2p_en' package is required for English text-to-phoneme conversion. "
            "Please install it using 'pip install g2p-en'."
        )

    # ARPAbet to IPA mapping (based on MFA's english_us_arpa phoneme set)
    arpabet_to_ipa = {
    ...
    for phone in arpabet_phones:
        if not phone:
            continue
        # Remove stress markers (0, 1, 2) and convert
        ...
        if base_phone in arpabet_to_ipa:
            ...
            # Add stress marker back
            ipa_phones.append(ipa_phone + stress)
        elif phone in [' ', '']:
            ipa_phones.append(' ')
        else:
            # Keep unknown phones as-is (like punctuation)
            pass
...
def convert_phones_to_tokens(
    phone_to_id: Dict[str, int], text: str
) -> Tuple[List[int], List[int], List[str]]:
    """
    Converts phoneme text into tokens and stress IDs based on a mapping.

    Args:
        phone_to_id (dict): A dictionary mapping phonemes to token IDs.
        text (str): A string containing phonemes, separated by spaces.
                    Each phoneme can optionally be followed by a digit representing stress (0, 1, 2).

    Returns:
        tuple: A tuple containing three lists:
            - token_ids (list): List of token IDs corresponding to the phonemes.
            - stress_ids (list): List of stress IDs (1=unstressed, 2=primary, 3=secondary).
    ...
    for k in text.split():
        # Handle special tokens like <sil>
        if k in phone_to_id:
            phonemes.append(k)
            token_ids.append(phone_to_id[k])
            stress_ids.append(1)  # default unstressed
            continue
        ...
        if k[-1].isdigit():
            ...
            if stress_level == 0:
                stress_id = 1  # unstressed = 1
            elif stress_level == 1:
                stress_id = 2  # primary stress = 2
            elif stress_level == 2:
                stress_id = 3  # secondary stress = 3
            else:
                stress_id = 1  # default = 1
            phone_key = k[:-1]
        else:
            stress_id = 1  # default unstressed
            phone_key = k
        ...
        while i < len(phone_key):
            ...
            for phoneme in sorted_phonemes:
                if phone_key[i:].startswith(phoneme):
                    ...
                    break
            if not matched:
                print(f"Unmatched sequence in token '{k}': '{phone_key[i:]}'")
                break
...
def main():
    ...
    group.add_argument(
        ...
        help=(
            ...
            "  - 'phones' for phoneme sequence (e.g., ARPAbet),\n"
            "  - 'ipa' for IPA notation.\n"
    ...
    parser.add_argument(
        ...
        type=str,
        default="text",
        choices=["text", "phones", "ipa"],
        help=(
            "Specify the format of the input text:\n"
            "  - 'text' for English text,\n"
            "  - 'phones' for phoneme sequence,\n"
            "  - 'ipa' for IPA notation.\n"
    ...
    model, words_to_phones, phone_to_token, speaker_info = load_model(
    ...
    token_ids, stress_ids, phonemes, phone_text = process_text(
        args.text, args.type, words_to_phones, phone_to_token
    )

    if args.silence:
        sil = phone_to_token.get("<sil>", phone_to_token.get("sil", 0))
    ...
    with torch.inference_mode():
        mel, dur, pitch, _ = model(speaker_id, token_ids, stress_ids)
    ...
    if args.spectogram:
        plot_mel_spectrogram(mel, pitch, dur, phonemes, phone_text, vocoder_predictor)
