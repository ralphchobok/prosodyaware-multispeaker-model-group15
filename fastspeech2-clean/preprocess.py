...
BATCH_SIZE = 512  # Reduced from 2048 to avoid CUDA OOM
...
def words_to_phones_stress(
    ...
    MFA marks stress with 0 (unstressed), 1 (primary stress), 2 (secondary stress).
    We map to: 1 (silence/unstressed), 2 (primary stress), 3 (secondary stress)
    """
    mapper = []
    stress = []
    for word in words_tier:
        ...
        for phone in phones_tier:
            if (
                phone["start_frames"] >= word["start_frames"]
                and phone["end_frames"] <= word["end_frames"]
            ):
                phone_str = phone["text"]
                if phone_str == SILENCE_TOKEN:
                    stress.append(1)  # silence = 1
                elif phone_str[-1].isdigit():  # Has stress marker
                    stress_level = int(phone_str[-1])
                    if stress_level == 0:
                        stress.append(1)  # unstressed = 1
                    elif stress_level == 1:
                        stress.append(2)  # primary stress = 2
                    elif stress_level == 2:
                        stress.append(3)  # secondary stress = 3
                    else:
                        stress.append(1)  # default = 1
                    phone_str = phone_str[:-1]  # Remove stress marker
                else:
                    stress.append(1)  # default unstressed = 1
...
def process_file(
    ...
    # MFA provides 'words' and 'phones' tiers for English
    ...
    torch.save(
        {
            ...
            "original_text": " ".join([k["text"] for k in words]),
        },
        output_filepath,
    )

    return speaker_info, phone_stats, words_stats, cur_phone_id


def create_statistics(
    speaker_info: Dict, phone_stats: List, words_stats: List, phone_to_id: Dict
) -> None:
    ...
    words_stats_df = (
        words_stats_df.groupby(["text", "phones"])
        .agg(
            occurrences=("text", "count"),
        )
        .sort_values("occurrences", ascending=False)
    ...
    words_stats_df.to_csv(output_path / "words.tsv", index=False, sep="\t")
...
def main():
    ...
    for filename in tqdm(textgrid_files):
        try:
            speaker_info, file_phone_stats, file_words_stats, cur_phone_id = (
    ...
    create_statistics(speaker_info, phone_stats, words_stats, phone_to_id)
