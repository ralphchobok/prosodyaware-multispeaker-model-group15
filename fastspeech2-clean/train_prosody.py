...
class EmotionalCustomDataset(Dataset):
    """
    Dataset with emotion labels extracted from file paths.
    
    Expected structure:
    processed/SPEAKER_ID/EMOTION/filename.pt
    """
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # Path structure: .../SPEAKER_ID/EMOTION/filename.pt
        file_path = Path(self.files[idx])
        emotion_str = file_path.parent.name  # Get emotion folder name
        
        # Map emotion string to ID
        if emotion_str in EMOTION_TO_ID:
            data['emotion'] = EMOTION_TO_ID[emotion_str]
        else:
            # Default to Neutral if emotion not recognized
            logger.warning(f"Unknown emotion '{emotion_str}' in {self.files[idx]}, defaulting to Neutral")
...
def main():
    ...
    parser.add_argument("--pretrained", type=str, default="model.pt",
                      help="Path to pretrained model checkpoint")
    parser.add_argument("--from-scratch", action="store_true",
                      help="Train from scratch instead of fine-tuning")
    parser.add_argument("--output", type=str, default="model_prosody.pt",
    ...
    # Filter out LJ dataset files - only keep files in 4-digit speaker folders with emotion subfolders
    # File structure: processed/SPEAKER_ID/EMOTION/file.pt
    file_list = []
    for f in all_files:
        # Get speaker folder name (2 levels up from file)
        speaker_folder = os.path.basename(os.path.dirname(os.path.dirname(f)))
        # Only include files in 4-digit speaker folders (0011-0020)
        if speaker_folder.isdigit() and len(speaker_folder) == 4:
            file_list.append(f)
    ...
    # Extract speaker IDs from file paths: processed/SPEAKER_ID/EMOTION/file.pt
    # Get grandparent directory basename
    speaker_ids = np.asarray([os.path.basename(os.path.dirname(os.path.dirname(f))) for f in file_list])
