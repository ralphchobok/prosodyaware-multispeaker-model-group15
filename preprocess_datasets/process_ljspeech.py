"""
Process LJSpeech Dataset - Extract Features

This script extracts acoustic features from LJSpeech dataset that has been
...
Usage:
    ...
    python process_ljspeech.py --input ../preprocessed_datasets/LJ --output ../preprocessed_datasets/processed
"""
...
def process_file(filename: Path, speaker_info: Dict, phone_to_id: Dict, cur_phone_id: int, output_path: Path) -> Tuple[Dict, List, List, int]:
    ...
    speaker_id = "LJ"  # LJSpeech is single speaker
...
def main():
    parser = argparse.ArgumentParser(description="Process LJSpeech Dataset - Extract Features")
    parser.add_argument("--input", type=str, default="../preprocessed_datasets/LJ",
                       help="Path to LJSpeech TextGrid files")
    ...
    print("="*60)
    print("LJSPEECH FEATURE EXTRACTION")
    print("="*60)
    ...
    textgrid_files = sorted(input_path.glob("*.TextGrid"))
    ...
    for filename in tqdm(textgrid_files, desc="Processing files"):
        try:
            speaker_info, file_phone_stats, file_words_stats, cur_phone_id = process_file(
                filename, speaker_info, phone_to_id, cur_phone_id, output_path
            )
    ...
    phone_stats_df.to_csv(output_path / "phones_lj.tsv", index=False, sep="\t")
    
    words_stats_df = pd.DataFrame(words_stats)
    words_stats_df = words_stats_df.groupby(["text", "phones"]).agg(
        occurrences=("text", "count")
    ).sort_values("occurrences", ascending=False).reset_index()
    words_stats_df.to_csv(output_path / "words_lj.tsv", index=False, sep="\t")
    ...
    speaker_df.to_csv(output_path / "speakers_lj.tsv", index=False, sep="\t")
    
    print("\n" + "="*60)
    print("PROCESSING COMPLETE")
    print("="*60)
    print(f"Output directory: {output_path}")
    print(f"Files processed: {speaker_df['num_files'].sum()}")
