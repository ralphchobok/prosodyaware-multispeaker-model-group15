...
class CustomDataset(Dataset):
    ...
    def _compute_statistics(self):
        ...
        for filename in tqdm(self.files, desc="Computing statistics"):
            try:
                pt_file = torch.load(filename, weights_only=False)
...
def calculate_cer(asr_predictor, audio_file, original_text):
    # Temporarily disable deterministic algorithms for Whisper (not compatible with CUDA)
    deterministic_was_enabled = torch.are_deterministic_algorithms_enabled()
    if deterministic_was_enabled:
        torch.use_deterministic_algorithms(False)
    
    try:
        pred_text = asr_predictor.transcribe(
            audio=audio_file,
            language="en",
        )["text"]
    finally:
        # Re-enable deterministic algorithms if they were enabled before
        if deterministic_was_enabled:
            torch.use_deterministic_algorithms(True)
    
    # Basic normalization for English
    pred_text = pred_text.lower().strip()
    original_text = original_text.replace("<sil>", "").lower().strip()
    return np.clip(jiwer.cer(original_text, pred_text), 0.0, 1.0)


def calculate_mos(mos_predictor, wav_data, sampling_rate):
    # Temporarily disable deterministic algorithms for MOS predictor (not compatible with CUDA)
    deterministic_was_enabled = torch.are_deterministic_algorithms_enabled()
    if deterministic_was_enabled:
        torch.use_deterministic_algorithms(False)
...
def main():
    ...
    speaker_dict, unique_speakers = parse_speakers(
        os.path.join(OUTPUT_PATH, "speakers_lj.tsv")
    )
    ...
    words_df = pd.read_csv(
        f"{OUTPUT_PATH}/words_lj.tsv", sep="\t", keep_default_na=False
    )
    words_dict = words_df.set_index("text")["phones"].to_dict()

    phone_df = pd.read_csv(f"{OUTPUT_PATH}/phones_lj.tsv", sep="\t", keep_default_na=False)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # Save evaluation results to JSON after each epoch
    ...
    model.load_state_dict(torch.load("model.pt", weights_only=False)["state_dict"])
